...
(* === LaTeX mathpartir package proof export === *)
...
let rec latex_of_proof : proof -> string = function
  | Rule (s,pl) -> Printf.sprintf "\\inferrule*{%s}{%s}\n" (latex_of_proof_list pl) (latex_of_sequent s)

and latex_of_proof_list : proof list -> string = function
  | [h] -> latex_of_proof h
  | h::t -> Printf.sprintf "%s \\\\ %s" (latex_of_proof h) (latex_of_proof_list t)
  | _ -> " " (* for the sep to appear *)

let export_proof : string -> proof -> unit = fun fn p ->
  let f = open_out fn in
  Printf.fprintf f "\\documentclass[varwidth=40cm]{standalone}\n";
  Printf.fprintf f "\\usepackage{mathpartir}\n";
  Printf.fprintf f "\\begin{document}\n";
  Printf.fprintf f "\\begin{mathpar}\n";
  Printf.fprintf f "%s\n" (latex_of_proof p);
  Printf.fprintf f "\\end{mathpar}\n";
